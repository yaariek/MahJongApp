...
src/app/      ← screens (Expo Router). Imports from src/core, never the reverse.
...
with no simulator and so it could move to a server later. Vitest is configured
(`vitest.config.mts`) to only look at `src/core/**/*.test.ts`.
...
npm test             # vitest run  — src/core only
