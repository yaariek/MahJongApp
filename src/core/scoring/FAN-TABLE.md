...
| Pattern                                              |  番 | Status                                                        |
| ---------------------------------------------------- | --: | ------------------------------------------------------------- |
| 無花 (no flowers; else 無字花/大平糊 override)       |   1 | done                                                          |
| 正花 (flower number == seat number), per flower      |   2 | done                                                          |
| 爛花 (flower number != seat number), per flower      |   1 | done                                                          |
| 碰出東/南/西/北 — 正風 (門風 or 圈風)                |   2 | done                                                          |
| 碰出東/南/西/北 — 客風 (neither)                     |   1 | done                                                          |
| 碰出中/發/白 (番子刻)                                |   2 | done                                                          |
| 聽牌 (叮)                                            |   5 | needs wait                                                    |
| 雞糊 (own 番 total == 1 → becomes 10; finalize pass) |  10 | done                                                          |
| 對碰 (雙碰待)                                        |   1 | needs wait                                                    |
| 假獨                                                 |   1 | needs wait                                                    |
| 獨獨 (單釣)                                          |   2 | needs wait                                                    |
| 平糊                                                 |   3 | done                                                          |
...
| 無字 (no honors)                                     |   1 | done                                                          |
| 無字花 (no 字, no 花, not 平糊)                      |   5 | done                                                          |
| 大平糊 (無字花 + 平糊; replaces 平糊)                |  10 | done                                                          |
| 海底撈月                                             |  20 | needs flags                                                   |
...
| 間間糊 (自摸 門前 對對糊; replaces 對對糊 + 不求人)  | 100 | done                                                          |
...
2. **雞糊** — "胡出時（不計莊前）只得一番": the hand's own patterns total exactly
   **1 番** (excluding 連莊/拉莊). Such a hand scores 雞糊 = 10, **replacing** that
   1 番. (Implemented — `finalize` pass in `index.ts`.)
...
5. **間間糊** — 自摸 + 門前 + 對對糊 = 100, **replacing** 對對糊 + 不求人.
   (Implemented — evaluator emits the line, `finalize` strips the components.)
...
9. **無字 / 無花 / 無字花 / 大平糊 — one mutually-exclusive ladder, top tier only:**
   ...
   (Implemented — `closingLines` in `index.ts`, using `hand.flowers`; the old
   standalone `mouZi` evaluator was removed.)
